...
# make dev   restarts backend (:8790) + frontend (:5173) with the latest code
...
dev: stop ## Restart backend + frontend dev servers with the latest code
	@echo "==> api  http://127.0.0.1:$(BACKEND_PORT)  (jevloop serve, uv editable install)"
	@echo "==> web  http://localhost:5173             (vite dev, proxies /api -> $(BACKEND_PORT))"
	pnpm dev

stop: ## Stop anything listening on the backend/frontend ports
	@pids=$$(lsof -nP -t -iTCP:$(BACKEND_PORT) -iTCP:$(FRONTEND_PORTS) -sTCP:LISTEN 2>/dev/null || true); \
	if [ -n "$$pids" ]; then \
